...
1. Add validations to `Author` such that...
  [x]1. The name cannot be blank
  [x]1. The e-mail is unique
1. Add validations to `Post` such that...
  [x]1. The title cannot be blank
  [x]1. The category is either `"Fiction"` or `"Non-Fiction"`
  [x]1. The content is at least 100 characters long.
[x]1. Change `AuthorsController#create` to re-render the form if the new author is invalid.
